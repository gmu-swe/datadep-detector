...
cbVMStart(jvmtiEnv *jvmti, JNIEnv *env) {
	...
	enter_critical_section(jvmti);
	{
		...
		static JNINativeMethod registry[2] = { { "_setTag",
				"(Ljava/lang/Object;Ledu/gmu/swe/datadep/DependencyInfo;)V",
				(void*) &setObjExpression }, { "_getTag",
				"(Ljava/lang/Object;)Ledu/gmu/swe/datadep/DependencyInfo;",
				(void*) &getObjExpression } };
		/* Register Natives for class whose methods we use */
		klass = env->FindClass("edu/gmu/swe/datadep/TagHelper");
		if (klass == NULL) {
			fatal_error(
					"ERROR: JNI: Cannot find edu.gmu.swe.datadep.TagHelper with FindClass\n");
		}
		rc = env->RegisterNatives(klass, registry, 2);
		if (rc != 0) {
			fatal_error(
					"ERROR: JNI: Cannot register natives for class edu.gmu.swe.datadep.TagHelper\n");
		}
		/* Engage calls. */
